...
export function TeamEditor({
  ...
  onClose: () => void;
  onSaved: (id?: string, updated?: TeamRecord) => void;
}) {
  ...
  function updateSlots(next: EditorSlot[]) {
    ...
    setDraft((d) => ({
      ...
      set_editing: next.map((s) => s.editing || null),
    }));
  }
  function reconcile() {
    ...
    setDraft((d) => ({
      ...d,
      set_notes: next.slots.map((s) => s.note),
      set_editing: next.slots.map((s) => s.editing || null),
    }));
    setOrphans((old) => [
      ...old,
  ...
  function patch(p: Partial<PokemonSet>, manualAttackIv = false) {
    updateSlots(
      slots.map((s, i) =>
        i === active
          ? patchAuthoredSlot(
  ...
  async function save(asNewVersion = false) {
    ...
    try {
      ...
      if (mode === 'metadata') {
        const updated = await api('patch', { id: team!.id, patch: finalDraft });
        endBuilderTiming('save-persisted');
        onSaved(team!.id, updated);
      } else if (mode === 'version') {
        const current =
          team!.history?.find((v) => v.id === team!.current_version_id) ||
          team!.version;
        const updated = await api(asNewVersion ? 'version' : 'save', {
          id: team!.id,
          draft: finalDraft,
          expected: team!.current_version_id,
          expected_revision: current.edit_revision || current.id,
      ...
      >
        {mode === 'metadata' ? (
          ...
          <div className="visual-team-workspace">
            ...
            {editorMode === 'visual' ? (
              <>
                <PokemonSlotBar
                  ...
                  onAdd={() => {
                    if (slots.length < 24) {
                      startBuilderTiming('add-pokemon');
              ...
              <div className="raw-team-mode">
                ...
                {rawWarning && (
                  <output className="warning">{rawWarning}</output>
                )}
              </div>
            )}
            {orphans.length > 0 && (
              <section className="orphan-notes">
                ...
                <p>
                  The text changed which Pokémon these notes belonged to. Choose
                  a slot above to attach each note, or discard it explicitly.
                </p>
                {orphans.map((n) => (
                  <div key={n.id}>
            ...
            <details className="team-metadata-panel">
              <summary>New version comment (optional)</summary>
              <Field label="Version comment">
                <input
                  ...
                  placeholder="Optional note for Save as new version"
                />
              </Field>
            </details>
          </div>
        ...
        <div className="modal-actions editor-save">
          {mode === 'version' && (
            <button
              className="button"
              disabled={busy || !draft.showdown_text.trim()}
          ...
          <button
            ...
            disabled={
              busy ||
              historical ||
              (mode !== 'metadata' && !draft.showdown_text.trim())
            }
            onClick={() => void save(false)}
            title={
              historical
                ? 'Historical versions are fixed. Save as new version to use these changes.'
                : undefined
            }
          >
            <Save size={15} />
