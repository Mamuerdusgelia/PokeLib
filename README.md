...
4. Enable Email authentication and keep **Confirm email** enabled. PokéLib uses native Supabase email/password signup and login. Keep the existing Resend/Supabase custom SMTP configuration for signup confirmation and password recovery; normal password login sends no email.
...
6. Production already has those same two runtime variables in Sites. Runtime values are not stored in .openai/hosting.json. Keep production owner-only until a separate public-release request.
7. Restart local development after changing environment values.
8. Log in with email and password. New users choose Create an account, enter and confirm their password, and confirm their email once. New accounts begin with an empty library.
...
In Supabase **Authentication → URL Configuration**, use Site URL `https://pokelib.app` and allowed redirects `https://pokelib.app/` and `http://localhost:3000/`. The application sends production signup and reset links to `https://pokelib.app/`, including requests made from the historical Sites hostname. Only the exact `http://localhost:3000` development origin gets localhost redirects. The root page handles the native Supabase implicit-flow callback, including `PASSWORD_RECOVERY`, and clears token/error fragments after the SDK processes them. A non-secret `?auth=reset` marker preserves the reset screen on reload; a valid Supabase session is still required to change a password. No `/auth/callback` or custom password/token backend is added.

**Existing accounts:** the owner ran a read-only password-presence check before this change and reported that both test accounts exist, have confirmed email, and have a password. Password hashes were never returned. Existing users can log in with their current password, even if it predates the new-password form's eight-character minimum. Anyone who does not know their password, including a magic-link-created user with no password, chooses **Forgot password?**, enters the same email address, follows the recovery email, and sets a password. Supabase updates the same Auth user; their team ownership does not change. Do not delete or recreate an existing account to migrate it.

Signup and new-password forms check matching confirmation and an eight-character minimum; Supabase enforces the project's server-side password policy. Password-reset requests show the same conditional message for existing and nonexistent addresses, including account-specific provider/rate-limit failures. Password change uses native `updateUser({ password })`, then signs out the current session and asks for the new password. The signed-in Settings panel also provides Set or reset password. Recovery emails should be opened in the intended browser; old/used links show a clear request-new-link message. The owner-only Sites gate still applies on both hostnames. Do not make the site public to test email flows.

Reference: [Supabase password authentication](https://supabase.com/docs/guides/auth/passwords), [password security](https://supabase.com/docs/guides/auth/password-security), and [redirect configuration](https://supabase.com/docs/guides/auth/redirect-urls). The existing custom SMTP service is preserved; the default-service quota encountered in the earlier magic-link session is historical, not evidence about the current Resend setup.
